...
To install **Cast**, follow these steps:

1. **Install Foundry** (if not installed already):
   - First, install Foundry by running the following command in your terminal:
     ```bash
     curl -L https://foundry.paradigm.xyz | bash
     ```
   - Once installed, run the following command to install Foundry's components (including Cast):
     ```bash
     foundryup
     ```

2. **Verify the installation**:
   - After installation, check if Cast is installed by running:
     ```bash
     cast --version
     ```
